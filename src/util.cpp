...
const char * const BITCOIN_CONF_FILENAME = "telnetcoin.conf";
const char * const BITCOIN_PID_FILENAME = "telnetcoind.pid";
...
#else
    const char* pszModule = "telnetcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Telnetcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Telnetcoin";
#else
    // Unix
    return pathRet / ".telnetcoin";
#endif
#endif
}
