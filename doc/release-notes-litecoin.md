Telnetcoin Core version 0.18.1 is now available from:

  <https://download.telnetcoin.org/telnetcoin-0.18.1/>
...
`/Applications/Telnetcoin-Qt` (on Mac) or `telnetcoind`/`telnetcoin-qt` (on
Linux).
...
Telnetcoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Telnetcoin Core on unsupported systems.
...
10.10. Additionally, Telnetcoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/minblock/telnetcoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:11334:11334` (this is an extra `:11334` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@telnetcoin.org) the Telnetcoin Core
...
- A new short [document](https://github.com/minblock/telnetcoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/minblock/telnetcoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/minblock/telnetcoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/minblock/telnetcoin/blob/master/doc/telnetcoin-conf.md)
  about the `telnetcoin.conf` file describes how to use it to configure
  Telnetcoin Core.

- A new document introduces Telnetcoin Core's BIP174 [Partially-Signed
  Telnetcoin Transactions
  (PSBT)](https://github.com/minblock/telnetcoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/minblock/telnetcoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Telnetcoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Telnetcoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/minblock/telnetcoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/minblock/telnetcoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Telnetcoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Telnetcoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/minblock/telnetcoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/minblock/telnetcoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart telnetcoind with the `-deprecatedrpc=generate`
...
- A new `telnetcoin-wallet` tool is now distributed alongside Telnetcoin
...
This section describes planned changes to Telnetcoin Core that may affect
other Telnetcoin software and services.

- Since version 0.16.0, Telnetcoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Telnetcoin Core 0.20 (expected about
  a year after 0.18), Telnetcoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Telnetcoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Telnetcoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Telnetcoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Telnetcoin Core
...
- A sub-project of Telnetcoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Telnetcoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Telnetcoin Core's own implementation, although entropy
  gathered by Telnetcoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Telnetcoin Core a
...
- On macOS, Telnetcoin Core now opts out of application CPU throttling
...
- #11334 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
